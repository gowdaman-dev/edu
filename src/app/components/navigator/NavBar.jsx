...
function NavBar() {
    ...
    useEffect(() => {
        fetch('/api/schoolList', {
            ...
            cache: 'no-store', next: { revalidate: 0 }
        }).then((data) => data.json()).then((values) => { setSchoolFilterData(values); console.log(values); }).catch((err) => console.log("fetching data"))
    }, [schoolfiltertoggle])
                        ...
                        <div className="flex items-center justify-center gap-4" >
                            <div className="relative flex items-center justify-center">
                                <div className=" relative flex flex-col justify-right">
                                    <button
                                        onClick={() => { setSchoolFilterToggle(!schoolFilterToggle); console.log(schoolFilterToggle); }}
                                        ref={schoolFilterRef}
