...
import {
  transporter,
  mailOption,
} from "@/app/components/mailcomponent/nodemailer";
  ...
  try {
    if (event === 0) {
      ...
      return NextResponse.json({
        message: "Member request deleted successfully",
      });
    }
    if (event === 1) {
      const { name, email, schoolname, grade, role } =
        await MemberRequest.findById(accid);
      await transporter.sendMail({
        ...mailOption,
        to: email,
...
<body>
  <div class="container">
    ...
    <p>Best regards,<br>The MiWay Team</p>
      ...
      return NextResponse.json({
        message: "User created and member request deleted successfully",
      });
    } else {
